...
export class NotificationController {
  ...
  async markAllReadController(
    req: CombinedAuthRequest,
    res: Response,
    _next: NextFunction,
  ) {
    const updated = await this.notificationService.markAllAsRead(this.getAuthEntity(req));
    res.status(200).json({ success: true, data: { updated } });
  }

  async markReadController(
  ...
  async saveFcmTokenController(
    ...
    await this.notificationService.saveFcmToken(ownerId, ownerType, {
      ...body,
      fcmToken,
    });
  ...
  async getNotificationByIdController(
    req: CombinedAuthRequest<{ id: string }>,
    res: Response,
    _next: NextFunction,
  ) {
