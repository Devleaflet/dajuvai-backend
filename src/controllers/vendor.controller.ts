...
export class VendorController {
    ...
    async getVendors(_req: VendorAuthRequest, res: Response, _next: NextFunction): Promise<void> {
        const vendors = await this.vendorService.fetchAllVendors();
        res.status(200).json({ success: true, data: vendors });
    }

    async getPartialVendors(_req: VendorAuthRequest, res: Response, _next: NextFunction): Promise<void> {
        const vendors = await this.vendorService.fetchPartialVendors();
        res.status(200).json({ success: true, data: vendors });
    }

    async getUnapprovedVendorList(_req: AuthRequest, res: Response, _next: NextFunction): Promise<void> {
    ...
    async vendorSignup(req: VendorAuthRequest<{}, {}, IVendorSignupRequest>, res: Response, _next: NextFunction): Promise<void> {
        const parsed = vendorSignupSchema.safeParse(req.body);
        if (!parsed.success) {
            throw new ValidationError("Validation failed", parsed.error.errors.map(e => ({ field: e.path.join("."), message: e.message })));
        }
        ...
        if (existingVendor && existingVendor.isVerified) throw new ConflictError("Vendor already exists");

        const districtEntity = await this.districtService.findDistrictByName(district);
        if (!districtEntity) throw new BadRequestError("District does not exist");
    ...
    async login(req: VendorAuthRequest<{}, {}, IVendorLoginRequest>, res: Response, _next: NextFunction): Promise<void> {
        ...
        if (!vendor.isApproved) {
            throw new ForbiddenError("Your account is not yet approved. You can only login once an admin approves you as a vendor.");
        }
    ...
    async refreshToken(req: Request, res: Response, _next: NextFunction): Promise<void> {
        ...
        const decoded = jwt.verify(token, config.JWT_REFRESH_SECRET) as { id: number; email: string; businessName: string };
    ...
    async sendVerificationToken(req: VendorAuthRequest<{}, {}, IVerificationTokenRequest>, res: Response, _next: NextFunction): Promise<void> {
        ...
        if (vendor.resendBlockUntil && vendor.resendBlockUntil > now) {
            ...
            throw new RateLimitError(`Too many verification attempts. Please try again in ${remainingMinutes} minute(s).`);
        }
        ...
        res.status(202).json({ success: true, message: 'Verification token sent' });
    }

    async forgotPassword(req: VendorAuthRequest<{}, {}, IVerificationTokenRequest>, res: Response, _next: NextFunction): Promise<void> {
        const parsed = verificationTokenSchema.safeParse(req.body);
        if (!parsed.success) {
            throw new ValidationError("Validation failed", parsed.error.errors.map(e => ({ field: e.path.join("."), message: e.message })));
        }
        ...
        res.status(202).json({ success: true, message: 'Password reset request sent' });
    }

    async resetPassword(req: VendorAuthRequest<{}, {}, IResetPasswordRequest>, res: Response, _next: NextFunction): Promise<void> {
    ...
    async getVendorById(req: VendorAuthRequest<{ id: string }>, res: Response, _next: NextFunction): Promise<void> {
    ...
    async authVendor(req: VendorAuthRequest, res: Response, _next: NextFunction): Promise<void> {
    ...
    async updateVendor(req: VendorAuthRequest<{ id: string }, {}, Partial<IUpdateVendorRequest>>, res: Response, _next: NextFunction): Promise<void> {
    ...
    async approveVendor(req: AuthRequest<{ id: string }>, res: Response, _next: NextFunction): Promise<void> {
        const vendorId = req.params.id;

        const isValid = await this.vendorService.findVendorById(Number(vendorId));
        if (!isValid.isVerified) throw new BadRequestError("Vendor must be verified");
    ...
    async deleteVendor(req: AuthRequest<{ id: string }, {}, {}, {}>, res: Response, _next: NextFunction): Promise<void> {
    ...
    async vendorSignupV2(req: VendorAuthRequest<{}, {}, IVendorSignupRequestV2>, res: Response, _next: NextFunction): Promise<void> {
        ...
        await sendVerificationEmail(data.email, "Vendor Email Verification", verificationToken);
    ...
    async updateVendorV2(req: VendorAuthRequest<{ id: string }, {}, Partial<IUpdateVendorRequestV2>>, res: Response, _next: NextFunction): Promise<void> {
    ...
    async updatePaymentOption(
        req: VendorAuthRequest<{ vendorId: string; paymentOptionId: string }, {}, Partial<IUpdateVendorPaymentOptionRequest>>,
        res: Response,
        _next: NextFunction
    ): Promise<void> {
        ...
        const updated = await this.vendorService.updatePaymentOptionService(vendorId, paymentOptionId, req.body);

        res.status(200).json({ success: true, message: "Payment option updated successfully", data: updated });
    }
}
