...
export class HomePageSectionController {
   ...
   */
  createHomePageSection = async (
    req: Request<{}, {}, ICreateHomepageSectionInput, {}>,
    res: Response,
    _next: NextFunction,
  ): Promise<void> => {
    const data: ICreateHomepageSectionInput = req.body;

    const existingSection = await this.homePageSectionService.checkByTitle(
      data.title,
    );
    if (existingSection)
      throw new ConflictError("Section with this title already exists");

    switch (data.productSource) {
      case ProductSource.MANUAL: {
        const uniqueProductIds = [
          ...new Set((data.productIds || []).map(Number)),
        ].filter((id) => Number.isFinite(id));

        if (uniqueProductIds.length) {
          const productsCount = await AppDataSource.getRepository(
            Product,
          ).count({
        ...
        const homepageManual =
          await this.homePageSectionService.createHomePageSection(data);
      ...
      case ProductSource.DEAL: {
        const dealExists = await this.dealService.handleGetDealById(
          data.selectedDealId,
        );
  ...
  updateHomePageSection = async (
    ...
    if (!id || isNaN(Number(id)))
      throw new BadRequestError("Valid section ID is required");
    ...
    const section =
      await this.homePageSectionService.updateHomePageSection(updatePayload);
  ...
  getAllHomePageSections = async (
    req: Request<{}, {}, {}, { search: string; includeInactive: boolean }>,
    res: Response,
    _next: NextFunction,
  ): Promise<void> => {
    const includeInactiveRaw = req.query.includeInactive;
    const includeInactiveBool =
      includeInactiveRaw?.toString().toLowerCase() === "true";

    const sections = await this.homePageSectionService.getAllHomePageSections(
      includeInactiveBool,
      req.query.search,
    );
    ...
    const ratingsMap =
      await this.reviewService.getBatchAverageRatings(allProductIds);
  ...
  getHomePageSectionById = async (
    ...
    const section = await this.homePageSectionService.getHomePageSectionById(
      Number(id),
    );
  ...
  deleteHomePageSection = async (
    ...
    const result = await this.homePageSectionService.deleteHomePageSection(
      Number(id),
    );
  ...
  toggleSectionStatus = async (
    ...
    const section = await this.homePageSectionService.toggleSectionStatus(
      Number(id),
    );
