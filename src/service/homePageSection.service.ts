...
import {
    ICreateHomepageSectionInput,
    IUpdateHomePageSectionInput,
...
export class HomePageSectionService {
     ...
     */
    constructor() {
        this.homepageSectionRepository =
            AppDataSource.getRepository(HomePageSection);
    ...
    async createHomePageSection(data: ICreateHomepageSectionInput) {
        ...
        switch (productSource) {
            ...
            case ProductSource.DEAL:
                const deal =
                    await this.dealService.handleGetDealById(selectedDealId);
    ...
    async updateHomePageSection(data: IUpdateHomePageSectionInput) {
        ...
        if (isActive !== undefined) {
            console.log(
                `✏️ Updating isActive: '${section.isActive}' → '${isActive}'`,
            );
        ...
        if (productSource !== undefined && productSource !== null) {
            console.log(
                `🔄 Updating productSource: '${section.productSource}' → '${productSource}'`,
            );
            ...
            switch (productSource) {
                case ProductSource.MANUAL: {
                    ...
                    if (!productIds || productIds.length === 0) {
                        throw new APIError(
                            400,
                            "At least one product must be selected for manual source",
                        );
                    }
                    const products = await this.productRepository.findBy({
                        id: In(productIds),
                ...
                case ProductSource.CATEGORY: {
                    console.log(
                        "📂 Updating CATEGORY with categoryId:",
                        selectedCategoryId,
                    );
                ...
                case ProductSource.SUBCATEGORY: {
                    console.log(
                        "📂 Updating SUBCATEGORY with categoryId:",
                        selectedCategoryId,
                        "subcategoryId:",
                        selectedSubcategoryId,
                    );
                    if (!selectedCategoryId || !selectedSubcategoryId) {
                        throw new APIError(
                            400,
                            "Both category and subcategory are required",
                        );
                    }
                    ...
                    if (!subcategory)
                        throw new APIError(
                            404,
                            "Selected subcategory does not exist or does not belong to category",
                        );
                ...
                case ProductSource.DEAL: {
                    console.log(
                        "💰 Updating DEAL with dealId:",
                        selectedDealId,
                    );
                    if (!selectedDealId)
                        throw new APIError(400, "Deal is required");
                    const deal =
                        await this.dealService.getDealById(selectedDealId);
                    console.log("🔎 Found deal:", deal?.id);
                    if (!deal)
                        throw new APIError(404, "Selected deal does not exist");
                    section.selectedDeal = deal;
                    section.products = [];
                    section.selectedCategory = null;
                    section.selectedSubcategory = null;
                ...
                default: {
                    console.error(
                        "❌ Invalid productSource received:",
                        productSource,
                    );
                    throw new APIError(400, "Invalid product source");
                }
    ...
    async getAllHomePageSections(
        ...
        const query =
            this.homepageSectionRepository.createQueryBuilder("section");
        ...
        const subcategorySections = sections.filter(
            (s) =>
                s.productSource === ProductSource.SUBCATEGORY &&
                s.selectedSubcategory?.id,
        );
        const subcategoryProductsMap = new Map<number, Product[]>();
        if (subcategorySections.length) {
            const subcategoryIds = subcategorySections.map(
                (s) => s.selectedSubcategory.id,
            );
        ...
        const categorySections = sections.filter(
            (s) =>
                s.productSource === ProductSource.CATEGORY &&
                s.selectedCategory?.id,
        );
        const categoryProductsMap = new Map<number, Product[]>();
        if (categorySections.length) {
            const categoryIds = categorySections.map(
                (s) => s.selectedCategory.id,
            );
        ...
        return sections.map((section) => {
            ...
            switch (section.productSource) {
                case ProductSource.DEAL:
                    products = (
                        dealProductsMap.get(section.selectedDeal?.id) ?? []
                    ).slice(0, 25);
                    break;

                case ProductSource.CATEGORY:
                    products = (
                        categoryProductsMap.get(section.selectedCategory?.id) ??
                        []
                    ).slice(0, 25);
                    break;
                ...
                case ProductSource.SUBCATEGORY:
                    products = (
                        subcategoryProductsMap.get(
                            section.selectedSubcategory?.id,
                        ) ?? []
                    ).slice(0, 25);
                    break;
