...
export class OrderService {
  ...
  private async getDistrict(districtName: string): Promise<District> {
    if (!districtName) throw new APIError(400, "Customer district is required");
    ...
    const district = await this.shippingService.resolveDistrictByName(districtName);
  ...
  private async getOrCreateAddress(
    ...
    const resolvedDistrict = await this.shippingService.resolveDistrictByName(
  ...
  private createOrderItems(items: any[]): OrderItem[] {
    return items.map((item) => {
      ...
      return this.orderItemRepository.create({
        ...
        imageSnapshot: item.variant?.variantImages?.[0] || item.product.productImages?.[0] || null,
  ...
  private async createOrderEntity(
    ...
    const { discountAmount, appliedPromoCode } = await this.calculateDiscount(
      userId,
      orderData.promoCode,
      merchandiseSubtotal,
      shippingTotal,
  ...
  async estimateCheckout(
    ...
    request: Pick<
      IOrderCreateRequest,
      "shippingAddress" | "promoCode" | "isBuyNow" | "productId" | "variantId" | "quantity"
    >,
  ) {
    const { shippingAddress, promoCode, isBuyNow, productId, variantId, quantity } = request;
    ...
    const customerDistrict = await this.shippingService.resolveDistrictByName(
      shippingAddress?.district,
    );
    if (!customerDistrict) {
    ...
    const { vendorShippingBreakdown, shippingTotal } =
      this.shippingService.calculateOrderShipping(
        { districtId: customerDistrict.id, districtName: customerDistrict.name },
        vendorGroups.map((g) => ({
          vendorId: g.vendorId,
          vendorDistrict: { districtId: g.vendorDistrictId, districtName: g.vendorDistrictName },
        })),
      );
    ...
    return {
      ...
      vendorShippingBreakdown: vendorShippingBreakdown.map((vs) => {
        const group = vendorGroups.find((g) => g.vendorId === vs.vendorId)!;
        return {
  ...
  async createOrder(
    ...
    try {
      ...
      if (idempotencyKey) {
        ...
        if (existingOrder) {
          return { order: existingOrder, vendorids: [], useremail: "", esewaRedirectUrl: undefined };
        }
      }
      ...
      const { vendorShippingBreakdown, shippingTotal } =
        this.shippingService.calculateOrderShipping(
          { districtId: address.districtId, districtName: address.district },
          vendorGroups.map((g) => ({
            vendorId: g.vendorId,
            vendorDistrict: { districtId: g.vendorDistrictId, districtName: g.vendorDistrictName },
          })),
      ...
      if (paymentMethod === PaymentMethod.CASH_ON_DELIVERY) {
        ...
        order = await this.reserveStockAndSaveOrder(order, vendorShippingRows);
  ...
  async esewaFailed(orderId: number) {
    try {
      ...
      order.deliveryStatus = DeliveryStatus.DELIVERY_FAILED;
      await this.orderRepository.save(order);
      await this.recordStatusChange(order.id, previousStatus, OrderStatus.CANCELLED, {
  ...
  async orderSuccess(orderId: number, transactionId: string) {
    try {
      ...
      await this.orderRepository.save(order);
      await this.recordStatusChange(order.id, previousStatus, OrderStatus.CONFIRMED, {
  ...
  private async validateStock(cartItems: CartItem[]): Promise<void> {
    const variantIds = [
      ...new Set(
        cartItems
          .map((item) => (item.variant ? Number(item.variant.id) : null))
          .filter(
  ...
  private async restoreStock(
    ...
    const variantIds = [
      ...new Set(
        orderItems
          .map((item: any) => (item.variantId ? Number(item.variantId) : null))
          .filter(
            (id: any): id is number =>
  ...
  private async reserveStockAndSaveOrder(
    ...
    return await AppDataSource.transaction(async (manager) => {
      ...
      const vendorShippingRepo = manager.getRepository(OrderVendorShipping);
  ...
  async verifyPayment(
    ...
    await this.recordStatusChange(order.id, previousStatus, order.status, {
      reason: isSuccessful ? "Payment verified" : "Payment verification failed",
    });
    if (isSuccessful) {
      await this.notificationService.notifyPaymentSuccess(
        order.id,
  ...
  > {
    ...
    for (const item of items) {
      ...
      groups.set(vendor.id, {
        ...
        vendorDistrictId: vendor.districtId ?? vendor.district.id ?? null,
  ...
  private groupOrderItemsByVendor(orderItems: OrderItem[]) {
    ...
    for (const item of orderItems) {
      ...
      if (!vendor || !vendor.district || !vendor.district.name) {
        throw new APIError(400, `Vendor for order item ${item.id} has no valid address`);
      }
  ...
  async updateShippingAddress(
    ...
    const { vendorShippingBreakdown, shippingTotal } =
      this.shippingService.calculateOrderShipping(
        { districtId: shippingAddress.districtId, districtName: shippingAddress.district },
        vendorGroups.map((g) => ({
          vendorId: g.vendorId,
          vendorDistrict: { districtId: g.vendorDistrictId, districtName: g.vendorDistrictName },
        })),
